...
class mk_XL():
    ...
    def ep4(self):
        ...
        ans = [ID, f"Организация: {a[0][0]}   ФИО представителя: {a[0][1]}   Тел.: {a[0][2]}   email: {a[0][3]}", "", mark, a[0][4], str(current_date)]
...
class DB():
    ...
    def get_params(self, mark, flc_type = "", Hm = "", V = "", sh = "", flc = "", fz = ""):
        ...
        if flc_type == "" or flc_type == None:
            ...
            sheet = WB["Связь"]
            for i in range(2, 998):
                if (str(sheet[f"A{i}"].value) == mark) and (str(sh) == str(sheet[f'B{i}'].value)) and (str(flc) == str(sheet[f'c{i}'].value)) and (str(Hm) == str(sheet[f'D{i}'].value)) and (str(V) == str(sheet[f'E{i}'].value)):
                    if sheet[f'M{i}'].value not in ans:
                         ans.append(Sheet[f"M{i}"].value)
